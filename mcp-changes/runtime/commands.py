...
class Commands(object):
    ...
    def readconf(self):
        ...
        self.md5reobfclientsource = config.get('REOBF', 'MD5PreReobfClientSource')
        self.md5reobfserversource = config.get('REOBF', 'MD5PreReobfServerSource')
    ...
    def gathermd5ssource(self, side, reobf=False):
        if not reobf:
            md5lk     = {0:self.md5client,    1:self.md5server}
        else:
            md5lk     = {0:self.md5reobfclientsource,    1:self.md5reobfserversource}
        pathbinlk = {0:self.binclient,    1:self.binserver}

        md5file = open(md5lk[side],'w')

        #HINT: We pathwalk the recompiled classes
        for path, dirlist, filelist in os.walk("./src/minecraft/net/minecraft/src"):
            for bin_file in glob.glob(os.path.join(path, '*.java')):
                bin_file_osindep = os.sep.join(bin_file.replace(os.sep,'/').split('/')[2:]).split('.')[0]
                md5file.write('%s %s\n'%(bin_file_osindep, md5(open(bin_file,'rb').read()).hexdigest()))
        md5file.close()
    ...
    def unpackreobfsource(self, side):
        jarlk     = {0:self.reobfjarclient, 1:self.reobfjarserver}
        md5lk     = {0:self.md5client,      1:self.md5server}
        md5reoblk = {0:self.md5reobfclientsource, 1:self.md5reobfserversource}
        outpathlk = {0:self.dirreobfclt,    1:self.dirreobfsrv}
