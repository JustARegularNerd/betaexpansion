    ...
    implements Runnable
{
        ...
        try
        {
            ...
            do
            {
                ...
                try
                {
                    ...
                    RenderBlocks.fancyGrass = gameSettings.g_Grass == 2 || (gameSettings.fancyGraphics && gameSettings.g_Grass != 1);
                    RenderBlocks.grassSide = gameSettings.fancyGrass;
                    ...
                    if(mod_BetaExpansion.debugMode)
                    {
                    	if(Keyboard.isKeyDown(64))
                    	{
                    		theWorld.setWorldTime(theWorld.getWorldTime()-50L);
                    	}
                    	if(Keyboard.isKeyDown(65))
                    	{
                    		theWorld.setWorldTime(theWorld.getWorldTime()+50L);
    ...
    {
    	/*if(Keyboard.isKeyDown(42) && Keyboard.isKeyDown(60) && mod_BetaExpansion.debugMode)
    	{
    		if(count != -1)
    		{
        		theWorld.setBlock((int)thePlayer.posX, (int)thePlayer.posY, (int)thePlayer.posZ, Block.workbench.blockID);
	            isTakingScreenshot = true;
	            GuiCrafting test = new GuiCrafting(thePlayer.inventory, theWorld, (int)thePlayer.posX, (int)thePlayer.posY, (int)thePlayer.posZ);
	            java.util.List l = CraftingManager.getInstance().getRecipeList();
	            if(count >= l.size())
	            {
	            	String s = ((IRecipe)l.get(count-1)).getRecipeOutput().getItemName();
	            	if(s.startsWith("item.") || s.startsWith("tile."))
	            	{
	            		s = s.substring(5);
	            	}
	            	s = "crafting-"+s;
	            	ingameGUI.addChatMessage(ScreenShotHelper.saveScreenshot(minecraftDir, displayWidth, displayHeight, s, true));
	            	count = -1;
	            	return;
	            }
	            IRecipe o = (IRecipe)l.get(count);
	            if(o instanceof ShapedRecipes)
	            {
	            	ShapedRecipes s = ((ShapedRecipes)o);
            		int offset = 0;
            		if(s.recipeWidth == 1 && s.recipeHeight == 2)
            		{
            			offset = 4;
            		}
            		if(s.recipeWidth == 2 && s.recipeHeight == 2)
            		{
            			offset = 4;
            		}
            		if(s.recipeWidth == 3 && s.recipeHeight == 2)
            		{
            			offset = 3;
            		}
            		if(s.recipeWidth == 3 && s.recipeHeight == 1)
            		{
            			offset = 6;
            		}
	            	for(int x = 0; x < 9; x++)
	            	{
	            		if((x) % 2 == 1 && s.getRecipeSize() == 2 && s.recipeWidth == 1 && s.recipeHeight == 2)
	            		{
	            			offset+=2;
		            		test.inventorySlots.putStackInSlot(x+1+offset, x < s.getRecipeSize() ? s.recipeItems[x] : null);
	            		}else
	            		if((x) % 3 == 2 && s.getRecipeSize() == 4 && s.recipeWidth == 2 && s.recipeHeight == 2)
	            		{
	            			offset++;
		            		test.inventorySlots.putStackInSlot(x+1+offset, x < s.getRecipeSize() ? s.recipeItems[x] : null);
	            		}else
	            ...
	            {
	            	ShapelessRecipes s = ((ShapelessRecipes)o);
	            	int offset = 0;
	            	if(s.getRecipeSize() == 1)
	            	{
	            		offset = 4;
	            	}
	            	if(s.getRecipeSize() == 2)
	            	{
	            		offset = 4;
	            	}
	            	if(s.getRecipeSize() == 3)
	            	{
	            		offset = 1;
	            	}
	            	if(s.getRecipeSize() == 4)
	            	{
	            		offset = 4;
	            	}
	            	for(int i = 0; i < s.getRecipeSize(); i++)
	            	{
	            		if(i % 2 == 1 && s.getRecipeSize() == 2)
	            		{
	            			offset += 2;
	            		}else
	            		if(i % 3 == 2 && s.getRecipeSize() == 3)
	            		{
	            			offset += 2;
	            		}else
	            		if(i % 3 == 2 && s.getRecipeSize() == 4)
	            		{
	            			offset++;
	            		}
            			test.inventorySlots.putStackInSlot(i+1+offset, i < s.getRecipeSize() ? (ItemStack)s.recipeItems.get(i) : null);
	            	}
	            }
	            displayGuiScreen(test);
	            test.updateScreen();
	            if(count != 151)
	            {
	            	String s = ((IRecipe)l.get(count-1)).getRecipeOutput().getItemName();
        ...
        if(Keyboard.isKeyDown(gameSettings.keyScreenShot.keyCode))
        {
    ...
    public void runTick()
    {
        ...
        renderEngine.getTexture("/betaexpansion/be_blocks.png");
        if(!isGamePaused)
        {
        ...
        if(currentScreen == null || currentScreen.field_948_f)
        {
            do
            {
                ...
                if(l <= 200L)
                {
                    ...
                    if(currentScreen == null)
                    {
                        ...
                        {
                            if(Mouse.getEventButton() - 100 == gameSettings.keyBindAttack.keyCode && Mouse.getEventButtonState())
                            {
                                clickMouse(0);
                                mouseTicksRan = ticksRan;
                            }
                            if(Mouse.getEventButton() - 100 == gameSettings.keyBindUse.keyCode && Mouse.getEventButtonState())
                            {
                                clickMouse(1);
                                mouseTicksRan = ticksRan;
                            }
                            if(Mouse.getEventButton() - 100 == gameSettings.keyBindPickItem.keyCode && Mouse.getEventButtonState())
                            {
            ...
            do
            {
                ...
                if(Keyboard.getEventKey() == gameSettings.keyBindAttack.keyCode && Keyboard.getEventKeyState())
                {
                    clickMouse(0);
                    mouseTicksRan = ticksRan;
                }
                if(Keyboard.getEventKey() == gameSettings.keyBindUse.keyCode && Keyboard.getEventKeyState())
                {
                    clickMouse(1);
                    mouseTicksRan = ticksRan;
                }
                if(Keyboard.getEventKey() == gameSettings.keyBindPickItem.keyCode && Keyboard.getEventKeyState())
                {
                    clickMiddleMouseButton();
                }
                if(Keyboard.getEventKeyState())
                {
                    if(Keyboard.getEventKey() == gameSettings.keyFullscreen.keyCode)
                    {
                    ...
                    } else
                    {
                        ...
                        } else
                        {
                        	if(mod_BetaExpansion.debugMode)
                        	{
                        		...
                        		{
                        			theWorld.getWorldInfo().setRaining(!theWorld.getWorldInfo().getRaining());
                        		}
                        		if(Keyboard.getEventKey() == 35)
                        		{
                        			thePlayer.attackEntityFrom(null, 2);
                        		}
                        		...
                        		if(Keyboard.getEventKey() == 37)
                        		{
                        			...
                        			{
                        				if(!(theWorld.loadedEntityList.get(i) instanceof EntityPlayer))
                        				{
                        					((Entity)theWorld.loadedEntityList.get(i)).setEntityDead();
                        				}
                        			}
                        		}
                        		if(Keyboard.getEventKey() == 46)
                        		{
                        			thePlayer.inventory = new InventoryPlayer(thePlayer);
                        			thePlayer.inventorySlots = new ContainerPlayer(thePlayer.inventory, !theWorld.multiplayerWorld);
                        		}
                        		if(Keyboard.getEventKey() == 47)
                        		{
                        			renderTexOverlay = !renderTexOverlay;
                        		}
                        		if(Keyboard.getEventKey() == 49)
                        		{
                        			for(int i = 0; i < 256; i++)
                        			{
                        				int x = i%16;
                        				int y = i/16;
                        				theWorld.setBlock((int)thePlayer.posX+x, (int)thePlayer.posY-1, (int)thePlayer.posZ+y, Block.blocksList[i] != null ? Block.blocksList[i].blockID : 0);
                        			}
                        		}
                        		if(Keyboard.getEventKey() == 48)
                        		{
                        			EntityItem e = new EntityItem(theWorld, thePlayer.posX, thePlayer.posY, thePlayer.posZ, new ItemStack(Item.pickaxeDiamond,1));
                        			EntityItem e1 = new EntityItem(theWorld, thePlayer.posX, thePlayer.posY, thePlayer.posZ, new ItemStack(Item.axeDiamond,1));
                        			EntityItem e2 = new EntityItem(theWorld, thePlayer.posX, thePlayer.posY, thePlayer.posZ, new ItemStack(Item.shovelDiamond,1));
                        			EntityItem e3 = new EntityItem(theWorld, thePlayer.posX, thePlayer.posY, thePlayer.posZ, new ItemStack(Item.swordDiamond,1));
                        			EntityItem e4 = new EntityItem(theWorld, thePlayer.posX, thePlayer.posY, thePlayer.posZ, new ItemStack(Item.hoeDiamond,1));
                        			theWorld.entityJoinedWorld(e);
                        			theWorld.entityJoinedWorld(e1);
                        			theWorld.entityJoinedWorld(e2);
                        			theWorld.entityJoinedWorld(e3);
                        			theWorld.entityJoinedWorld(e4);
                        		}
                        	}
                            ...
                            if(Keyboard.getEventKey() == gameSettings.keyToggleGui.keyCode)
                            {
                                gameSettings.hideGUI = !gameSettings.hideGUI;
                            }
                            if(Keyboard.getEventKey() == gameSettings.keyDebug.keyCode)
                            {
                                gameSettings.showDebugInfo = !gameSettings.showDebugInfo;
                            }
                            if(Keyboard.getEventKey() == gameSettings.keyTogglePerspective.keyCode)
                            {
                                gameSettings.thirdPersonView = !gameSettings.thirdPersonView;
                            }
                            if(Keyboard.getEventKey() == gameSettings.keySmoothCamera.keyCode)
                            {
                        ...
                        for(int i = 0; i < 9; i++)
                        {
                            if(Keyboard.getEventKey() == gameSettings.hotbarKeys[i].keyCode)
                            {
            ...
            if(currentScreen == null)
            {
                if((gameSettings.keyBindAttack.keyCode < 0 ? Mouse.isButtonDown(gameSettings.keyBindAttack.keyCode + 100) : Keyboard.isKeyDown(gameSettings.keyBindAttack.keyCode)) && (float)(ticksRan - mouseTicksRan) >= timer.ticksPerSecond / 4F && inGameHasFocus)
                {
                    clickMouse(0);
                    mouseTicksRan = ticksRan;
                }
                if((gameSettings.keyBindUse.keyCode < 0 ? Mouse.isButtonDown(gameSettings.keyBindUse.keyCode + 100) : Keyboard.isKeyDown(gameSettings.keyBindAttack.keyCode)) && (float)(ticksRan - mouseTicksRan) >= timer.ticksPerSecond / 4F && inGameHasFocus)
                {
                    clickMouse(1);
                    mouseTicksRan = ticksRan;
                }
            }
            func_6254_a(0, currentScreen == null && (gameSettings.keyBindAttack.keyCode < 0 ? Mouse.isButtonDown(gameSettings.keyBindAttack.keyCode + 100) : Keyboard.isKeyDown(gameSettings.keyBindAttack.keyCode)) && inGameHasFocus);
        }
        if(theWorld != null)
        {
            if(thePlayer != null)
    	...
    	if(lockTime)
    	{
    		theWorld.setWorldTime(theWorld.getWorldTime()-1);
    	}
        systemTime = System.currentTimeMillis();
    ...
    public static void startMainThread(String s, String s1, String s2)
    {
        boolean flag = false;
        String s3 = s;
        mcFrame = new Frame("Minecraft");
        Canvas canvas = new Canvas();
        mcFrame.setLayout(new BorderLayout());
        mcFrame.add(canvas, "Center");
        canvas.setPreferredSize(new Dimension(854, 480));
        mcFrame.pack();
        mcFrame.setLocationRelativeTo(null);
        MinecraftImpl minecraftimpl = new MinecraftImpl(mcFrame, canvas, null, 854, 480, flag, mcFrame);
        ...
        mcFrame.addWindowListener(new GameWindowListener(minecraftimpl, thread));
        thread.start();
    }
    ...
    public boolean renderTexOverlay;
    public static Frame mcFrame;
}
