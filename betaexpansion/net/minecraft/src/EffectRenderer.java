...
public class EffectRenderer
{
    ...
    public void renderParticles(Entity entity, float f)
    {
        ...
        for(int i = 0; i < 5; i++)
        {
            ...
            if(i == 4)
            {
                j = renderer.getTexture(BETextureHelper.block_path);
            }
            ...
            for(int k = 0; k < fxLayers[i].size(); k++)
            {
                ...
                tessellator.setBrightness(entityfx.getRenderBrightness(f));
        ...
        for(int k1 = 0; k1 < j1; k1++)
        {
            for(int l1 = 0; l1 < j1; l1++)
            {
                for(int i2 = 0; i2 < j1; i2++)
                {
                    ...
                    addEffect((new EntityDiggingFX(worldObj, i, j, k, d, d1, d2, d - (double)i - 0.5D, d1 - (double)j - 0.5D, d2 - (double)k - 0.5D, block, j2, i1)).func_4041_a(i, j, k));
                }
    ...
    public void addBlockHitEffects(int i, int j, int k, int l)
    {
        ...
        addEffect((new EntityDiggingFX(worldObj, i, j, k, d, d1, d2, 0.0D, 0.0D, 0.0D, block, l, worldObj.getBlockMetadata(i, j, k))).func_4041_a(i, j, k).func_407_b(0.2F).func_405_d(0.6F));
    }

    public String getStatistics()
    {
        return (new StringBuilder()).append("").append(fxLayers[0].size() + fxLayers[1].size() + fxLayers[2].size() + fxLayers[3].size()).toString();
    }
