    ...
    implements IWorldAccess
{
    ...
    public void loadRenderers(boolean completeReload)
    {
        ...
        if(mc.theWorld != null && worldRenderers != null)
        {
        ...
        if(worldRenderers == null || completeReload)
        {
        ...
        for(int j1 = 0; j1 < renderChunksWide; j1++)
        {
            for(int k1 = 0; k1 < renderChunksTall; k1++)
            {
                ...
                {
                	int x = (l1 * renderChunksTall + k1) * renderChunksWide + j1;
                	if(worldRenderers[x] == null || completeReload)
                	{
                		worldRenderers[x] = new WorldRenderer(worldObj, tileEntities, j1 * 16, k1 * 16, l1 * 16, 16, glRenderListBase + k);
                	}
                    if(occlusionEnabled)
                    {
                        worldRenderers[x].glOcclusionQuery = glOcclusionQueryBase.get(l);
                    }
                    worldRenderers[x].isWaitingOnOcclusionQuery = false;
                    ...
                    k += worldRenderers[x].renderPasses + 1;
                }
    ...
    public void renderEntities(Vec3D vec3d, ICamera icamera, float f)
    {
        ...
        for(int j = 0; j < list.size(); j++)
        {
            ...
            if(!entity1.isInRangeToRenderVec3D(vec3d) || !entity1.ignoreFrustumCheck && !icamera.isBoundingBoxInFrustum(entity1.boundingBox, false) || entity1 == mc.renderViewEntity && !mc.gameSettings.thirdPersonView && !mc.renderViewEntity.isPlayerSleeping())
            {
                continue;
            }
    ...
    public int sortAndRender(EntityLiving entityliving, int i, double d)
    {
    	...
    	if(i == 2)
    	{
    		GL11.glBindTexture(GL11.GL_TEXTURE_2D,renderEngine.getTexture("/betaexpansion/be_blocks.png"));
    	}
        for(int j = 0; j < 10; j++)
        {
        ...
        int k = 0;
        if(occlusionEnabled && mc.gameSettings.advancedOpengl > 0 && !mc.gameSettings.anaglyph && (i == 0))
        {
            int l = 0;
            int i1 = 16;
            ...
            do
            {
                ...
                for(int k1 = byte0; k1 < i1; k1++)
                {
                    ...
                    float f3 = MathHelper.sqrt_float(worldrenderer1.distanceToEntitySquared(entityliving));
                    ...
                    if(!worldrenderer1.isPartiallyOutsideFrustum)
                    {
                    	worldrenderer1.isVisible = true;
                        continue;
                    }
            ...
            double d3, double d4, double d5)
    {
        ...
        if(s.equals("smoke") && mc.gameSettings.p_smoke)
        {
        ...
        if(s.equals("explode") && mc.gameSettings.p_explode)
        {
            mc.effectRenderer.addEffect(new EntityExplodeFX(worldObj, d, d1, d2, d3, d4, d5));
        } else
        if(s.equals("flame") && mc.gameSettings.p_flame)
        {
        ...
        if(s.equals("largesmoke") && mc.gameSettings.p_smoke)
        {
            mc.effectRenderer.addEffect(new EntitySmokeFX(worldObj, d, d1, d2, d3, d4, d5, 2.5F));
        } else
        if(s.equals("reddust") && mc.gameSettings.p_reddust)
        {
    ...
    public void obtainEntitySkin(Entity entity)
    {
    	...
    	{
            entity.cloakUrl = (new StringBuilder()).append("http://skins.minecraft.net/MinecraftCloaks/").append(((EntityPlayer)entity).username).append(".png").toString();
    		((EntityPlayer)entity).playerCloakUrl = entity.cloakUrl;
            entity.skinUrl = (new StringBuilder()).append("http://skins.minecraft.net/MinecraftSkins/").append(((EntityPlayer)entity).username).append(".png").toString();
    	}else
    	{
    		entity.updateCloak();
    	}
        if(entity.skinUrl != null)
        {
        	if(entity instanceof EntityPlayer)
        	{
        		ImageBufferDownload d = new ImageBufferDownload();
                renderEngine.obtainImageData(entity.skinUrl, d);
                if(players.get(((EntityPlayer)entity).username) == null)
                {
                	players.put(((EntityPlayer)entity).username, d);
                }
        	}else
        	{
    ...
    public void releaseEntitySkin(Entity entity)
    {
        ...
        if(entity instanceof EntityPlayer)
        {
        	players.remove(((EntityPlayer)entity).username);
        }
    }
