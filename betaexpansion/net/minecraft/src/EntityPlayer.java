...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    {
    	boolean f = inventory.getStackInSlot(ItemSpyglass.slotUsed) == null || inventory.getCurrentItem() == null || inventory.getCurrentItem() != ItemSpyglass.stackUsed || inventory.getCurrentItem().itemID != BEItems.spyGlass.shiftedIndex;
    	if(f)
    	{
    		EntityRenderer.zoom = false;
    	}
    	if(inventory.getStackInSlot(ItemSpyglass.slotUsed) == null)
    	{
    		EntityRenderer.zoomProgress = 1.0D;
    		EntityRenderer.prevZoomProgress = 1.0D;
    		zoomProgress = 1.0D;
    	}
    	boolean flag2 = ModLoader.getMinecraftInstance().gameSettings.thirdPersonView;
		EntityRenderer.prevZoomProgress = EntityRenderer.zoomProgress;
    	if(EntityRenderer.zoom)
    	{
    		if(!prevZoom)
    		{
    			prevZoom = true;
    		...
    		{
    			slowdownValue *= 0.7D;
    			zoomProgress += slowdownValue;
    		}else
    		{
    			speedValue *= 2.2D;
    			if(speedValue > 0.4D) speedValue = 0.4D;
    			zoomProgress += speedValue;
    		} 
    		if(zoomProgress >= 3.0D) 
    		{
    			startSlowdown = true;
    		}
    		if(zoomProgress > 6.0D) zoomProgress = 6.0D;
    		if(zoomProgress < 1.0D) zoomProgress = 1.0D;
    		EntityRenderer.zoomProgress = !flag2?zoomProgress:1.0F;
    		if(EntityRenderer.zoomProgress > 4.0D)
    		{
    			EntityRenderer.zoomProgress = 4.0D;
    		}
    	}else
    	{
    		if(prevZoom)
    		...
    		speedValue *= 1.6D;
    		if(speedValue > 0.5D) speedValue = 0.5D;
    		zoomProgress -= speedValue;
    		if(zoomProgress < 1.0D) zoomProgress = 1.0D;
    		EntityRenderer.zoomProgress = !flag2?zoomProgress:1.0F;
    		if(EntityRenderer.zoomProgress > 4.0D)
    ...
    public static ChunkCoordinates func_25060_a(World world, ChunkCoordinates chunkcoordinates)
    {
        ...
        ichunkprovider.prepareChunk(chunkcoordinates.x - 3 >> 4, chunkcoordinates.z + 3 >> 4);
        ichunkprovider.prepareChunk(chunkcoordinates.x + 3 >> 4, chunkcoordinates.z + 3 >> 4);
        int l = world.getBlockId(chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z);
        if(l != Block.blockBed.blockID && l != 114)
        {
            return null;
        } else
        {
        	ChunkCoordinates chunkcoordinates1;
        	if(l == Block.blockBed.blockID)
        	{
                chunkcoordinates1 = BlockBed.getNearestEmptyChunkCoordinates(world, chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z, 0);
        	}else
        	{
                chunkcoordinates1 = BlockCryingObsidian.getNearestEmptyChunkCoordinates(world, chunkcoordinates.x, chunkcoordinates.y, chunkcoordinates.z, 0);
        	}
    ...
    private static boolean prevZoom;
    private static double zoomProgress;
    private static boolean startSlowdown;
    private static double slowdownValue = 0.2D;
    private static double speedValue = 0.02D;
}
