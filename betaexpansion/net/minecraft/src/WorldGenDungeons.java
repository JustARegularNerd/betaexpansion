...
label0:
        ...
        placeCobwebs(world,random,i,j,k,l,i1);
        return true;
    }
    ...
    private ItemStack pickCheckLootItem(Random random)
    {
        ...
        {
            return new ItemStack(Item.itemsList[Item.record13.shiftedIndex + random.nextInt(12)]);
        }
        if(i == 10)
        {
    ...
    private void placeCobwebs(World world, Random rand, int x, int y, int z, int i, int j)
    {
        for(int k1 = x - i - 1; k1 <= x + i + 1; k1++)
        {
            for(int j2 = y - 1; j2 <= y + 3 + 1; j2++)
            {
                for(int i3 = z - j - 1; i3 <= z + j + 1; i3++)
                {
                	if(world.getBlockId(k1, j2, i3) != 0)
                	{
                		continue;
                	}
                	int x1 = world.getBlockId(k1+1,j2,i3);
                	int x2 = world.getBlockId(k1-1,j2,i3);
                	int x3 = world.getBlockId(k1,j2,i3+1);
                	int x4 = world.getBlockId(k1,j2,i3-1);
                	int x5 = world.getBlockId(k1,j2+1,i3);
                	int x6 = world.getBlockId(k1,j2-1,i3);
                	if(world.getBlockId(k1,j2,i3) == 0 && rand.nextInt(10) == 0 &&
                			((x1 != 0 && Block.blocksList[x1].blockMaterial.getIsSolid()) || (x2 != 0 && Block.blocksList[x2].blockMaterial.getIsSolid())
                		  || (x3 != 0 && Block.blocksList[x3].blockMaterial.getIsSolid()) || (x4 != 0 && Block.blocksList[x4].blockMaterial.getIsSolid()))
                			&& ((x5 != 0 || x6 != 0)))
                	{
                		world.setBlock(k1, j2, i3, 30);
                	}
                }
            }
        }
