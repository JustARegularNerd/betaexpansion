...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float f, boolean flag, int i, int j)
    {
        ...
        if(mc.gameSettings.g_Vignette == 2 || (mc.gameSettings.fancyGraphics && mc.gameSettings.g_Vignette != 1))
        {
            renderVignette(mc.thePlayer.getEntityBrightness(f),f, k, l);
        }
        ItemStack itemstack = mc.thePlayer.inventory.armorItemInSlot(3);
        if(mc.renderTexOverlay)
        {
            Tessellator tessellator = Tessellator.instance;
        	double s = 192.0D;
            GL11.glBindTexture(3553, mc.renderEngine.getTexture("/terrain.png"));
            tessellator.startDrawingQuads();
            tessellator.addVertexWithUV(0.0D, s, -90D, 0.0D, 1.0D);
            tessellator.addVertexWithUV(s, s, -90D, 1.0D, 1.0D);
            tessellator.addVertexWithUV(s, 0.0D, -90D, 1.0D, 0.0D);
            tessellator.addVertexWithUV(0.0D, 0.0D, -90D, 0.0D, 0.0D);
            tessellator.draw();
            GL11.glBindTexture(3553, mc.renderEngine.getTexture("/gui/items.png"));
            tessellator.startDrawingQuads();
            tessellator.addVertexWithUV(s, s, -90D, 0.0D, 1.0D);
            tessellator.addVertexWithUV(s*2, s, -90D, 1.0D, 1.0D);
            tessellator.addVertexWithUV(s*2, 0.0D, -90D, 1.0D, 0.0D);
            tessellator.addVertexWithUV(s, 0.0D, -90D, 0.0D, 0.0D);
            tessellator.draw();
        }
        ...
        if(mc.gameSettings.showDebugInfo)
        {
            ...
            int x = MathHelper.floor_double((double)((mc.thePlayer.rotationYaw * 4F) / 360F) + 0.5D) & 3;
            int x1 = x;
            switch(x1)
            {
            case 0: x1 = 3; break;
            ...
            drawString(fontrenderer, (new StringBuilder()).append("facing: ").append(x).append(" ("+directions[x]+")").append(" renderIndex: ").append(x1).toString(), 2, 88, 0xe0e0e0);
            int c1 = MathHelper.floor_double(mc.thePlayer.posX);
            int c2 = MathHelper.floor_double(mc.thePlayer.posY);
            int c3 = MathHelper.floor_double(mc.thePlayer.posZ);
            if (mc.theWorld != null && mc.theWorld.blockExists(c1, c2, c3))
            {
                Chunk chunk = mc.theWorld.getChunkFromBlockCoords(c1, c3);
                drawString(fontrenderer, (new StringBuilder()).append("bl: ").append(chunk.getSavedLightValue(EnumSkyBlock.Block, c1 & 0xf, c2, c3 & 0xf)).append(" sl: ").append(chunk.getSavedLightValue(EnumSkyBlock.Sky, c1 & 0xf, c2, c3 & 0xf)).append(" rl: ").append(chunk.getBlockLightValue(c1 & 0xf, c2, c3 & 0xf, 0)).toString(), 2, 96, 0xe0e0e0);
            }
            if (!mc.theWorld.multiplayerWorld)
            {
                drawString(fontrenderer, (new StringBuilder()).append("Seed: ").append(mc.theWorld.worldInfo.getRandomSeed()).toString(), 2, 112, 0xe0e0e0);
            }
            GL11.glPopMatrix();
        }
