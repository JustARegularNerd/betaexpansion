...
public class MapGenCaves extends MapGenBase
{
    ...
    protected void func_870_a(int i, int j, byte abyte0[], double d, double d1, 
            double d2)
    {
        releaseEntitySkin(i, j, abyte0, d, d1, d2, 1.0F + rand.nextFloat() * 6F, 0.0F, 0.0F, -1, -1, 0.5D, new Random(rand.nextLong()));
    }
            ...
            double d3, Random random)
    {
        ...
        for(; k < l; k++)
        {
            ...
            if(!flag && k == j1 && f > 1.0F)
            {
                releaseEntitySkin(i, j, abyte0, d, d1, d2, (float) (0.75*d6 + 0.25*d6*random.nextFloat() - 1.5), f1 - 1.570796F, f2 / 3F, k, l, 1.0D, new Random(random.nextLong()));
                releaseEntitySkin(i, j, abyte0, d, d1, d2, (float) (0.75*d6 + 0.25*d6*random.nextFloat() - 1.5), f1 + 1.570796F, f2 / 3F, k, l, 1.0D, new Random(random.nextLong()));
                return;
            }
            ...
            if((d8a * d8a + d9a * d9a) > (d10a + d11) * (d10a + d11))
            {
                return;
            }
    ...
    protected void func_868_a(World world, int i, int j, int k, int l, byte abyte0[])
    {
        ...
        for(int j1 = 0; j1 < i1; j1++)
        {
            ...
            for(int l1 = 0; l1 < k1; l1++)
            {
                ...
                releaseEntitySkin(k, l, abyte0, d, d1, d2, f2, f, f1, 0, 0, 1.0D, new Random(rand.nextLong()));
            }
