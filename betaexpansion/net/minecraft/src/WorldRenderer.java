...
public class WorldRenderer
{
    ...
    public WorldRenderer(World world, List list, int i, int j, int k, int l, int i1)
    {
        ...
        glRenderList = -1;
        isInFrustum = false;
        isPartiallyOutsideFrustum = false;
        skipRenderPass = new boolean[renderPasses];
        isVisible = true;
        isInitialized = false;
    ...
    public void updateRenderer()
    {
        ...
        for(int k1 = 0; k1 < renderPasses; k1++)
        {
    ...
    {
        isInFrustum = icamera.isBoundingBoxInFrustum(rendererBoundingBox, false);
        if(GameSettings.advancedOpengl == 2)
        {
        	isPartiallyOutsideFrustum = icamera.isBoundingBoxInFrustum(rendererBoundingBox, true);
        }else
        {
        	isPartiallyOutsideFrustum = false;
        }
    ...
    public boolean skipAllRenderPasses()
    {
        ...
        {
        	for(int i = 0; i < skipRenderPass.length; i++)
        	{
        		if(!skipRenderPass[i])
        		{
        			return false;
    ...
    public boolean isPartiallyOutsideFrustum;
