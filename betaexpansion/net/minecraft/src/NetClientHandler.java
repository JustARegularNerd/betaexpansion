...
public class NetClientHandler extends NetHandler
{
    ...
    public void handleBE(Packet231BetaExpansion p)
    {
    }
    
    public void handleSeason(Packet232Season p)
    {
    	worldClient.worldInfo.setSeason(p.seasons ? p.offset : -1);
    	worldClient.worldInfo.setSeasonLength(p.length);
    	worldClient.worldInfo.setSeasonOffset(p.offset);
    }
    ...
    public void handleVehicleSpawn(Packet23VehicleSpawn packet23vehiclespawn)
    {
        ...
        if(packet23vehiclespawn.type == 104)
        {
            obj = new EntityFallingSand(worldClient, d, d1, d2, BEBlocks.redSand.blockID);
        }
        if(obj != null)
        {
    ...
    { 
        if(packet2handshake.username.equals("--"))
        {
            ...
            System.out.println("Server is BetaExpansion");
            addToSendQueue(new Packet231BetaExpansion(mod_BetaExpansion.protocolVersion));
        }else
        if(packet2handshake.username.equals("-"))
        {
            addToSendQueue(new Packet1Login(mc.session.username, 14));
