    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int l)
    {
        ...
        if(world.canBlockBePlacedAt(blockID, i, j, k, false, l))
        {
            ...
            if(world.setBlockAndMetadataWithNotify(i, j, k, blockID, getPlacedBlockMetadata(itemstack.getItemDamage())))
            {
                ...
                world.playSoundEffect((float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, block.getStepSound(j1).func_1145_d(), (block.getStepSound(j1).getVolume() + 1.0F) / 2.0F, block.getStepSound(j1).getPitch() * 0.8F);
                itemstack.stackSize--;
            }
            return true;
        } else
