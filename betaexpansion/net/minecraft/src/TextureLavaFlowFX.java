...
public class TextureLavaFlowFX extends TextureFX
{
    ...
    public TextureLavaFlowFX(Minecraft mc)
    {
    	this();
    	init(mc);
    }
    ...
    {
        try {
        	if(mc.texturePackList.selectedTexturePack.getResourceAsStream("/custom_lava_flowing.png") != null)
        	{
        		BufferedImage b = ImageIO.read(mc.texturePackList.selectedTexturePack.getResourceAsStream("/custom_lava_flowing.png"));
    ...
    public void onTick()
    {
        ...
        if(isCustom)
        {
        	System.arraycopy(allImages[field_1139_k%allImages.length], 0, imageData, 0, allImages[field_1139_k%allImages.length].length);
        	return;
        }
        for(int i = 0; i < 16; i++)
        {
