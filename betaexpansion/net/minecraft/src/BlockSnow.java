...
public class BlockSnow extends Block
{
    ...
    public boolean blockActivated(World world, int i, int j, int k, EntityPlayer entityplayer)
    {
    	...
    	{
    		world.setBlockAndMetadataWithNotify(i, j, k, x+1<15?blockID:Block.blockSnow.blockID, x+1<15?x+1:8);
    ...
    public boolean canPlaceBlockAt(World world, int i, int j, int k)
    {
        ...
        if((l == 0 || !Block.blocksList[l].isOpaqueCube()) && !(l == 18 || l == 140))
        {
            return false;
        } else
        {
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        if((world.getSavedLightValue(EnumSkyBlock.Block, i, j, k) > 11 ||
        		(15 - world.calculateSkylightSubtracted(1.0F) > 11 && world.worldInfo.getSeason() != 3 && !world.getWorldChunkManager().getBiomeGenAt(i, k).getEnableSnow()))
        		&& random.nextInt(7) == 0 && m <= 7)
        {
            dropBlockAsItem(world, i, j, k, m);
            world.setBlockAndMetadataWithNotify(i, j, k, m!=0?blockID:0, m!=0?m-1:0);
        }
    }
