    ...
    implements IBlockAccess
{
    ...
    public MovingObjectPosition func_28105_a(Vec3D vec3d, Vec3D vec3d1, boolean flag, boolean flag1)
    {
        ...
        if(block != null)
        {
        	block.setBlockBoundsBasedOnState(this, l, i1, j1);
        	block.getCollisionBoundingBoxFromPool(this, l, i1, j1);
        }
        ...
        for(int l1 = 200; l1-- >= 0;)
        {
            ...
            if(block1 != null)
            {
            	block1.setBlockBoundsBasedOnState(this, l, i1, j1);
            	block1.getCollisionBoundingBoxFromPool(this, l, i1, j1);
            }
            if((!flag1 || block1 == null || block1.getCollisionBoundingBoxFromPool(this, l, i1, j1) != null) && j2 > 0 && block1.canCollideCheck(i2, flag))
            {
    ...
    {
    	if(worldInfo.getSeasonLength()==0)
    	{
    		worldInfo.setSeason(-1);
    		return;
    	}
    	if(worldInfo.getSeason()==-1)
    	{
    		return;
    	}
    ...
    public void tick()
    {
        ...
        {
            boolean flag = false;
            if(spawnHostileMobs && difficultySetting >= 2)
            {
    ...
    protected void updateBlocksAndPlayCaveSounds()
    {
        ...
        for(Iterator iterator = positionsToUpdate.iterator(); iterator.hasNext();)
        {
            ...
            if(rand.nextInt(16) == 0)
            {
                ...
                BiomeGenBase biomegenbase = getWorldChunkManager().getBiomeGenAt(j3 + k, j4 + i1);
                int max = mod_BetaExpansion.maxSnow.get(biomegenbase);
                if((biomegenbase.getEnableSnow() || (biomegenbase.canSpawnLightningBolt() && worldInfo.getSeason() == 3)) && j5 >= 0 && j5 < 128 && chunk.getSavedLightValue(EnumSkyBlock.Block, j3, j5, j4) < 10)
                {
                    int i6 = chunk.getBlockID(j3, j5 - 1, j4);
                    int k6 = chunk.getBlockID(j3, j5, j4);
                    if(func_27161_C() && k6 == Block.snow.blockID)
                    {
                    	int m = chunk.getBlockMetadata(j3, j5, j4);
                    	if((m & 7)+1<max)
                    	{
                    		setBlockAndMetadataWithNotify(j3+k, j5, j4+i1, Block.snow.blockID, m+1);
                    	}
                    }
                    ...
                    (getBlockMaterial(j3 + k + 1, j5 - 1, j4 + i1) != Material.water || getBlockMaterial(j3 + k - 1, j5 - 1, j4 + i1) != Material.water || 
                     getBlockMaterial(j3 + k, j5 - 1, j4 + i1 + 1) != Material.water || getBlockMaterial(j3 + k, j5 - 1, j4 + i1 - 1) != Material.water))
                    {
