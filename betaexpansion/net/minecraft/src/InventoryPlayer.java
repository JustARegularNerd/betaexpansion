    ...
    implements IInventory
{
    ...
    public float getStrVsBlock(Block block, int i)
    {
        float f = 1.0F;
        if(mainInventory[currentItem] != null)
        {
            f *= mainInventory[currentItem].getStrVsBlock(block, i);
        }
        return f;
    }
    ...
    {
        if(block.getBlockMaterial(i).getIsHarvestable())
        {
            return true;
        }
        ItemStack itemstack = getStackInSlot(currentItem);
        if(itemstack != null)
        {
            return itemstack.canHarvestBlock(block, i);
        } else
        {
            return false;
        }
