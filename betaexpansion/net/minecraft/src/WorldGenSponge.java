...
public class WorldGenSponge extends WorldGenerator
{
    ...
    {
        if(world.getBlockMaterial(i, j, k) != Material.water || j > 60)
        {
            return false;
        }
        ...
        double d4 = j - 5 + random.nextInt(3) + 2;
        double d5 = j - 5 + random.nextInt(3) + 2;
            ...
            for(int k2 = i1; k2 <= j1; k2++)
            {
                for(int l2 = k1; l2 <= l1; l2++)
                {
                    for(int i3 = i2; i3 <= j2; i3++)
                    {
                        ...
                        if(j3 == Block.dirt.blockID)
                        {
                            world.setBlock(k2, l2, i3, BEBlocks.wetSponge.blockID);
                        }
                    }
