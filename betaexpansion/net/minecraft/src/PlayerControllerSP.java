...
public class PlayerControllerSP extends PlayerController
{
    ...
    public boolean sendBlockRemoved(int i, int j, int k, int l)
    {
        ...
        boolean flag1 = mc.thePlayer.canHarvestBlock(Block.blocksList[i1],j1);
        if(itemstack != null)
        {
    ...
    public void clickBlock(int i, int j, int k, int l)
    {
        ...
        int i1 = mc.theWorld.getBlockId(i, j, k);
        int j1 = mc.theWorld.getBlockId(i, j, k);
        if(i1 > 0 && curBlockDamage == 0.0F)
        {
            Block.blocksList[i1].onBlockClicked(mc.theWorld, i, j, k, mc.thePlayer);
        }
        if(i1 > 0 && Block.blocksList[i1].blockStrength(mc.thePlayer,j1) >= 1.0F)
        {
    ...
    public void sendBlockRemoving(int i, int j, int k, int l)
    {
        ...
        if(i == field_1074_c && j == field_1073_d && k == field_1072_e)
        {
            ...
            curBlockDamage += block.blockStrength(mc.thePlayer,j1);
            if(field_1069_h % 4F == 0.0F && block != null)
            {
                mc.sndManager.playSound(block.getStepSound(j1).func_1145_d(), (float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, (block.getStepSound(j1).getVolume() + 1.0F) / 8F, block.getStepSound(j1).getPitch() * 0.5F);
            }
            field_1069_h++;
