...
public class EntityRenderer
{
    ...
    private void setupCameraTransform(float f, int i)
    {
        ...
        if(zoomProgress > 1.0D)
        {
        	double actualZoom = prevZoomProgress + (zoomProgress - prevZoomProgress) * (double)f;
            GL11.glTranslatef((float)cameraYaw, (float)(-cameraPitch), 0.0F);
            GL11.glScaled(actualZoom, actualZoom, 1.0D);
    ...
    public void updateCameraAndRender(float f)
    {
        ...
        if(mc.inGameHasFocus)
        {
            ...
            if(mc.gameSettings.smoothCamera || zoomProgress > 1.0D)
            {
    ...
    public void renderWorld(float f, long l)
    {
        ...
        for(int i = 0; i < 2; i++)
        {
            ...
            renderglobal.sortAndRender(entityliving, 2, f);
    ...
    private void addRainParticles()
    {
        ...
        for(int i1 = 0; i1 < (int)(100F * f * f); i1++)
        {
            ...
            BiomeGenBase biomegenbase = world.getWorldChunkManager().getBiomeGenAt(j1, k1);
            if(l1 > j + byte0 || l1 < j - byte0 || (biomegenbase.canSpawnLightningBolt() && mc.theWorld.worldInfo.getSeason() == 3)
            		|| (!biomegenbase.canSpawnLightningBolt() && mc.theWorld.worldInfo.getSeason() != 3) || biomegenbase.getEnableSnow())
            {
                continue;
            }
    ...
    private void setupFog(int i, float f)
    {
        ...
        } else
        {
            ...
            GL11.glFogf(2915 /*GL_FOG_START*/, farPlaneDistance * 0.8F);
    ...
    public static boolean anaglyphEnabled = false;
