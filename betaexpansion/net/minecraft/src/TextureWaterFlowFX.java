...
public class TextureWaterFlowFX extends TextureFX
{
    ...
    public TextureWaterFlowFX(Minecraft mc)
    {
    	this();
    	init(mc);
    }
    ...
    {
        try {
        	if(mc.texturePackList.selectedTexturePack.getResourceAsStream("/custom_water_flowing.png") != null)
        	{
        		BufferedImage b = ImageIO.read(mc.texturePackList.selectedTexturePack.getResourceAsStream("/custom_water_flowing.png"));
        		res = b.getWidth();
        		imageData = new byte[res*res*4];
        		allImages = new byte[b.getHeight()/res][res*res*4];
        		for(int i = 0; i < b.getHeight()/res; i++)
        		{
        			int[] tmp = new int[res*res];
        			b.getRGB(0, i*res, res, res, tmp, 0, res);
    ...
    public void onTick()
    {
        ...
        if(isCustom)
        {
        	System.arraycopy(allImages[field_1134_k%allImages.length], 0, imageData, 0, allImages[field_1134_k%allImages.length].length);
        	return;
        }
        for(int i = 0; i < 16; i++)
        {
